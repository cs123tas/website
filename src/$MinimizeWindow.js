...
function $MinimizeWindow(options){
	...
	$w.$content.css({display: "none"})
	$w.$titlebar.css({width: "200"})
	...
	$w.minimize = function() {
		console.log($w.$content.css("display"))
		if($w.$content.css("display") === "none") {
			$w.$content.css({display: "block"})
			$w.$titlebar.css({width: ""})
		} else {
			$w.$content.css({display: "none"})
			$w.$titlebar.css({width: "200"})
		}
